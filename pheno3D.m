...
%         h1 = waitbar(0,'Current Plate:','Name','Plate Info');
        ...
        outputPrefix = ['AllChannel_NoBkg_SVVAL_' num2str(param.tileX) 'MVVAL_' num2str(param.megaVoxelTileX) '_SVCAT' num2str(svval(iSuperVoxel)) '_MVCAT' num2str(mvval(iMegaVoxel))];
...
%         Print All the inpur parameters
        ...
        for iPlates = 1:numPlates
            ...
            %     plateDirectory = 'Z:\PM_20160311_YASCPgmat_plate2\Meas01Tiff-Images';
            ...
            % [plateInfoTable, channelInfo]= getIC200info( plateDirectory );
            [plateInfoTable,metaDataHeader,channelInfo] = getPlateInfoFromMetadatafile(metadatafilename,param);
            
            % plateInfoTable = getAdditionalMetaInfo(plateInfoTable);
            %
            
%             waitbar(iPlates/numPlates,h1,['Current Plate: ' plateName],'Name','Plate Info'); % Update waitbar
            
            %     metaDataHeader  =  plateInfoTable(1,:);
            %     selHeader = true(1,length(metaDataHeader));
            %     for i = 1:length(channelDiscarded)
            %         selHeader = ~strcmpi(metaDataHeader,channelDiscarded{i,:}).*selHeader;
            %         channelInfo = channelInfo(~strcmpi(channelInfo,channelDiscarded{i,:}),:);
            %     end
            %     selHeader = logical(selHeader);
            %     metaDataHeader = metaDataHeader(1,selHeader);
            ...
            randFieldID = getTrainingFields(plateInfoTable,param,'TreatmentSum');
...
%             close(h2);
            ...
            if(~usePreviousParameterFile)
                ...
                intensityThreshold = zeros(numel(randFieldID).*param.origZ,param.numChannels);
            ...
            if(~usePreviousParameterFile)
                ...
                for iImages = 1:numel(randFieldID)
                    ...
                    pixelsForTraining(startVal:endVal,:) = getTrainingPixels(tmpInfoTable,param);
...
%             h2 = waitbar(0,'','Name','Final image analysis') ;
            ...
            for iImages = 1:numel(uniqueImageID)
                tImageAnal = tic;
                tmpInfoTable = plateInfoTable(allImageId == uniqueImageID(iImages),1:param.numChannels);
                [ superVoxelProfile,fgSuperVoxel,resultTAS(iImages,:) ] = getTileProfiles( tmpInfoTable, param.pixelBinCenters, param );
                [megaVoxelProfile,fgMegaVoxel] = getMegaVoxelProfile(superVoxelProfile,fgSuperVoxel,param);
